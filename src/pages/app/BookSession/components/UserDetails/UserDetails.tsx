...
export const UserDetails: React.FC<UserDetailsProps> = ({ onFilled, isLoggedIn }) => {
    const [state, dispatch] = useBookingReducer();
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

    // Walidacja tylko gdy użytkownik nie jest zalogowany
    const isEmailValid = !isLoggedIn && !!state.customerEmail && emailRegex.test(state.customerEmail);
